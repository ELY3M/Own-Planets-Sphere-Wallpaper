...
public class DiamondNumberSecondDialog extends Dialog implements OnClickListener {
    ...
    private class PositionView extends View implements OnSeekBarChangeListener, OnTouchListener {
        ...
        public void onDraw(Canvas canvas) {
            DiamondNumberSecondDialog.this.clockX = DiamondNumberSecondDialog.this.xBarValue;
            if (!DiamondNumberSecondDialog.this.moveable && DiamondNumberSecondDialog.this.touchToMove && System.currentTimeMillis() - DiamondNumberSecondDialog.this.touchTime >= 300) {
                Log.d(TAG, "!moveable");
                if (!DiamondNumberSecondDialog.this.vibrated) {
                    DiamondNumberSecondDialog.this.vibrator.vibrate(100);
                    DiamondNumberSecondDialog.this.vibrated = true;
                }
            }
        }

        public boolean onTouch(View v, MotionEvent event) {
            if (v == DiamondNumberSecondDialog.this.clock) {
                if (event.getAction() == 0 && !DiamondNumberSecondDialog.this.touchToMove) {
                    DiamondNumberSecondDialog.this.touchTime = System.currentTimeMillis();
                    DiamondNumberSecondDialog.this.touchToMove = true;
                    Log.d(TAG, "down");
                }
                if (DiamondNumberSecondDialog.this.touchToMove && System.currentTimeMillis() - DiamondNumberSecondDialog.this.touchTime >= 300) {
                    Log.d(TAG, "move");
                    if (!DiamondNumberSecondDialog.this.moveable && DiamondNumberSecondDialog.this.touchToMove && System.currentTimeMillis() - DiamondNumberSecondDialog.this.touchTime >= 300) {
                        DiamondNumberSecondDialog.this.moveable = true;
                    }
                    if (DiamondNumberSecondDialog.this.moveable && DiamondNumberSecondDialog.this.touchToMove) {
                        Log.d(TAG, "moveable");
                    }
                }
                if (event.getAction() == 1) {
                    Log.d(TAG, "up");
                    DiamondNumberSecondDialog.this.touchToMove = false;
                    DiamondNumberSecondDialog.this.moveable = false;
                    DiamondNumberSecondDialog.this.vibrated = false;
                }
            }
            return false;
        }
        ...
        public void onProgressChanged(SeekBar arg0, int arg1, boolean arg2) {
            IndividualWallpaperService.diamond2NumberValue = arg1;
        }
    ...
    DiamondNumberSecondDialog(Context context, DialogInterface.OnClickListener l, int defaultHand) {
        super(context);
    ...
    private void buildPortraitUI() {
        ...
        this.tx_xBar.setText(R.string.planetnum2summary);
        ...
        this.xBar.setProgress(IndividualWallpaperService.diamond2NumberValue);
    ...
    public void onClick(View v) {
        if (v == this.ok_btn) {
            IndividualWallpaperService.prefEditor.putInt(IndividualWallpaperService.Diamond2NumberValueKey, IndividualWallpaperService.diamond2NumberValue);
            IndividualWallpaperService.prefEditor.commit();
            IndividualWallpaperService.diamond2NumberValue = IndividualWallpaperService.prefs.getInt(IndividualWallpaperService.Diamond2NumberValueKey, 30);
            dismiss();
        }
        if (v == this.default_btn) {
            IndividualWallpaperService.diamond2NumberValue = 30;
            this.clockX = this.defaultX;
            this.xBarValue = 30;
            this.xBar.setProgress(IndividualWallpaperService.diamond2NumberValue);
        }
